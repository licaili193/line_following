...
class Car:
    def __init__(self,name,path,roadID,mergeID,FIFO,to,tfi,xo,xf,vo,vf,a,b,c):
        self.name = name
        self.path = path
        self.roadID = roadID
...
def controller_client(index,v_road,xPos,yPos,lv,rv): #rv,lv is zumo right/left velocity
    global L,S,s #control region,merge region
    ...
    if xPos>(mergePosx-(L+S)) and (mergePosy-laneWidth/2)<yPos<(mergePosy+laneWidth/2) and ((xPos-mergePosx)**2+(yPos-mergePosy)**2)**(1/2)<(L+S): #control region in same lane
        ...
        if mergeRegion == 0: #when a car first enters the control region calculate a,b,c
            ...
            if max(tempFIFO) == 0: #the first car sets to
                ...
                temptf[index]=tfi #save tfi for next car
                tempto[index]=to #save tfi for next car
                X_mtx = RT_control(0,tfi,xo,xf,vo,vf)
		rospy.loginfo("Orange" + str(to))
		...
		rospy.loginfo("test" + str(toi))
            ...
            t = time.time()/1000 - to
            rospy.loginfo("Zumo " +str(index+1)+" t "+str(t))
            v_target = 1/2*a*t**2 + b*t +c
            v_scale = 2*v_target/(rv+lv)
            rospy.loginfo("Zumo " +str(index+1)+" target velocity "+ str(v_target))
...
def lf_client():
    ...
    try:
        ...
        #Road 1 - starts on the bottom straight away (S94) and loops CCW
            if status == 0: #S94
                rospy.loginfo("Zumo " +str(index+1)+" on S94")
                if x<-0.9506: #T138
                    status = 1
            elif status == 1: #A106
                rospy.loginfo("Zumo " +str(index+1)+" on A106")
                if x<-2.276: #T144
                    status = 2
            elif status == 2: #A105/101/100
                rospy.loginfo("Zumo " +str(index+1)+" on A105/101/100")
                if x>-1.45 and y>0.3504: #T139
                    status = 3
            elif status == 3: #A99
                rospy.loginfo("Zumo " +str(index+1)+" on A99")
                if x>-0.8504: #T135
                    status = 4
...
def proc():
    ...
    for i in range(numCars):
        ...
        FIFO = carList[index].FIFO
        tfi = carList[index].tfi
        xo = carList[index].xo
        xf = carList[index].xf
        vo = carList[index].vo
        vf = carList[index].vf
        ...
        carList[index].FIFO = FIFO
        carList[index].tfi = tfi
        carList[index].xo = xo
        carList[index].xf = xf
        carList[index].vo = vo
        carList[index].vf = vf
